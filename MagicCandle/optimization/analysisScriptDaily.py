...
result_df = {}
filtered_df = {}
#read result from HDF store
#store = pd.HDFStore('nonhammer.h5')
#result_df[1] = store['nonhammer']
#store.close()

store = pd.HDFStore('DailyBacktest20170224.h5')
result_df[1] = store['daily']
store.close()

#store = pd.HDFStore('InsideBarCombBacktest20170220.h5')
#result_df[2] = store['daily']
#store.close()
...
final_df.groupby(['Curncy', 'Signal', 'Parameters', 'HoldingPeriod', 
                   ...
                   'MaxDrawdown']).sum().to_excel('DailySummary' + datetime.date.today().strftime('%Y%m%d') + '.xlsx')

# save signals
store = pd.HDFStore('DailySignal' + datetime.date.today().strftime('%Y%m%d') + '.h5')
...
for curncy in set(final_df.Curncy):
    ...
    excel = os.path.join(os.getcwd(), 'Daily', 'By Currency', curncy+'.xlsx')
...
for curncy in set(final_df.Curncy):
    for signal in signalList:
        ...
        if signalIndex.any() == True:
            ...
            excel = os.path.join(os.getcwd(), 'Daily', 'By Signal' ,signal, curncy+'.xlsx')
            if not os.path.exists(os.path.dirname(excel)):
                os.makedirs(os.path.dirname(excel))
            temp_df.to_excel(excel)
...
count_df.groupby(['Curncy', 'Signal']).sum().to_excel('DailyCount' + datetime.date.today().strftime('%Y%m%d') + '.xlsx')