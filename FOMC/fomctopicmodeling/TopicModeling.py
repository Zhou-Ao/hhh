# -*- coding: utf-8 -*-
"""
Created on Mon Jan 23 09:05:33 2017
...
def cleanDoc(doc, noun=False):
    print('Cleaning: ' + doc[:20])
    # tokenization
#    print('Cleaning ' + doc + '...')
...
def cleanDocNoun(doc):
    return cleanDoc(doc, noun = True)

clean_docs = map(cleanDocNoun, docs)
...
num_topics = 20
model = lda.LDA(n_topics=num_topics, n_iter=1000, random_state=1991)
...
topic_df.to_csv(folder + '_para ' + str(num_topics) + ' topic prob.csv')

# save training sample result

with open(folder + '_para ' + str(num_topics) + ' topic prob.txt', 'w') as f:
    for k in np.arange(0, len(docs)):
        for i, topic_dist in enumerate(topic_word):
             topic_words = np.array(vocab)[np.argsort(topic_dist)][:-n_top_words:-1]
             f.write('Topic {}: {}'.format(i, ' '.join(topic_words)))
             f.write(' ' + str(doc_topic[k][i]*100) + '\n')
        f.write(docs[k].encode('utf8') + '\n')



# topic estimation of fed minutes
fileNames_minutes = []
...
clean_minutes_docs = map(cleanDocNoun, minutes_docs)
...
doc_topic_test_pf.to_csv(folder + '_minutes_' + str(num_topics) +  ' topic prob_from para.csv')
